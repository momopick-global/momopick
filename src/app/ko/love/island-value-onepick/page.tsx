...
import { quizIslandValueOnepick } from "@/content/quiz";
import { BackButton } from "@/components/ko/BackButton";

const pack = quizIslandValueOnepick;
...
const pageDesc =
  pickQuizText(pageLocale, pack.meta?.description) || "무인도에 가져갈 물건 하나로, 연애에서 내가 가장 중요하게 여기는 가치를 알아보는 원픽 테스트.";
...
export const metadata: Metadata = {
  ...
  alternates: {
    canonical: "https://momopick.com/ko/love/island-value-onepick/",
  },
  openGraph: {
    title: pageTitle,
    description: ogDesc,
    url: "https://momopick.com/ko/love/island-value-onepick/",
    locale: "ko_KR",
    type: "website",
...
export default function IslandValueOnepickPage() {
  return (
    <>
      <KoSiteHeader />
      ...
      <div className="wrap">
        <main className="quiz-page">
          <nav className="quiz-breadcrumb" aria-label="경로">
            ...
            <span aria-hidden="true"> / </span>
            <span>무인도에 딱 하나 가져간다면?</span>
          </nav>
          <BackButton />

          <header className="quiz-page-hd">
            <p className="quiz-kicker">🎯 원픽 테스트</p>
            <h1 id="quiz-title">{pickQuizText(pageLocale, pack.title) || "무인도에 딱 하나 가져간다면?"}</h1>
