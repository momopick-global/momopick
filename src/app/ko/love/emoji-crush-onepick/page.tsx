...
import { quizEmojiCrushOnepick } from "@/content/quiz";
import { BackButton } from "@/components/ko/BackButton";

const pack = quizEmojiCrushOnepick;
...
const pageDesc =
  pickQuizText(pageLocale, pack.meta?.description) || "네 개의 이모지 중 지금 보내고 싶은 하나로, 썸 탈 때 나의 태도를 알아보는 원픽 테스트.";
...
export const metadata: Metadata = {
  ...
  alternates: {
    canonical: "https://momopick.com/ko/love/emoji-crush-onepick/",
  },
  openGraph: {
    title: pageTitle,
    description: ogDesc,
    url: "https://momopick.com/ko/love/emoji-crush-onepick/",
    locale: "ko_KR",
    type: "website",
...
export default function EmojiCrushOnepickPage() {
  return (
    <>
      <KoSiteHeader />
      ...
      <div className="wrap">
        <main className="quiz-page">
          <nav className="quiz-breadcrumb" aria-label="경로">
            ...
            <span aria-hidden="true"> / </span>
            <span>지금 보내고 싶은 이모지는?</span>
          </nav>
          <BackButton />

          <header className="quiz-page-hd">
            <p className="quiz-kicker">🎯 원픽 테스트</p>
            <h1 id="quiz-title">{pickQuizText(pageLocale, pack.title) || "지금 보내고 싶은 이모지는?"}</h1>
