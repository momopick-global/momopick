...
import { quizFlowerCharmOnepick } from "@/content/quiz";
import { BackButton } from "@/components/ko/BackButton";

const pack = quizFlowerCharmOnepick;
...
const pageDesc =
  pickQuizText(pageLocale, pack.meta?.description) || "여섯 송이 꽃 중 눈길이 먼저 가는 하나로, 나의 매력 포인트를 알아보는 원픽 테스트.";
...
export const metadata: Metadata = {
  ...
  alternates: {
    canonical: "https://momopick.com/ko/love/flower-charm-onepick/",
  },
  openGraph: {
    title: pageTitle,
    description: ogDesc,
    url: "https://momopick.com/ko/love/flower-charm-onepick/",
    locale: "ko_KR",
    type: "website",
...
export default function FlowerCharmOnepickPage() {
  return (
    <>
      <KoSiteHeader />
      ...
      <div className="wrap">
        <main className="quiz-page">
          <nav className="quiz-breadcrumb" aria-label="경로">
            ...
            <span aria-hidden="true"> / </span>
            <span>눈길이 먼저 가는 꽃은?</span>
          </nav>
          <BackButton />

          <header className="quiz-page-hd">
            <p className="quiz-kicker">🎯 원픽 테스트</p>
            <h1 id="quiz-title">{pickQuizText(pageLocale, pack.title) || "눈길이 먼저 가는 꽃은?"}</h1>
