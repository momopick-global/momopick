...
import { quizCafeSeatOnepick } from "@/content/quiz";
import { BackButton } from "@/components/ko/BackButton";

const pack = quizCafeSeatOnepick;
...
const pageDesc =
  pickQuizText(pageLocale, pack.meta?.description) || "카페에서 끌리는 자리 하나로, 관계에서 내가 두는 거리감을 알아보는 원픽 테스트.";
...
export const metadata: Metadata = {
  ...
  alternates: {
    canonical: "https://momopick.com/ko/love/cafe-seat-onepick/",
  },
  openGraph: {
    title: pageTitle,
    description: ogDesc,
    url: "https://momopick.com/ko/love/cafe-seat-onepick/",
    locale: "ko_KR",
    type: "website",
...
export default function CafeSeatOnepickPage() {
  return (
    <>
      <KoSiteHeader />
      ...
      <div className="wrap">
        <main className="quiz-page">
          <nav className="quiz-breadcrumb" aria-label="경로">
            ...
            <span aria-hidden="true"> / </span>
            <span>카페에서 고르는 자리는?</span>
          </nav>
          <BackButton />

          <header className="quiz-page-hd">
            <p className="quiz-kicker">🎯 원픽 테스트</p>
            <h1 id="quiz-title">{pickQuizText(pageLocale, pack.title) || "카페에서 고르는 자리는?"}</h1>
